...
create_xcframeworks() {
  ...
  libtool -static -o "$BUILD_DIR/arm64/libmozjpeg.a" \
      "$BUILD_DIR/arm64/libjpeg.a" \
      "$BUILD_DIR/arm64/libturbojpeg.a"

  libtool -static -o "$BUILD_DIR/universal/libmozjpeg.a" \
      "$BUILD_DIR/universal/libjpeg.a" \
      "$BUILD_DIR/universal/libturbojpeg.a"

    # Создаем libjpeg XCFramework
    echo "Creating libjpeg XCFramework..."
    xcodebuild -create-xcframework \
        -library "$BUILD_DIR/arm64/libmozjpeg.a" \
        -headers "$PUBLIC_DIR" \
        -library "$BUILD_DIR/universal/libmozjpeg.a" \
        -headers "$PUBLIC_DIR" \
        -output "$BUILD_DIR/frameworks/libmozjpeg.xcframework"
