...
do $$
begin
	insert into identities (id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at)
	...
	from users as users
	where invited_at is not null and not exists (select user_id from identities where user_id = users.id and provider = 'email');
end $$;
